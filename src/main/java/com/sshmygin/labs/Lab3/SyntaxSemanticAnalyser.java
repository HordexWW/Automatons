package com.sshmygin.labs.Lab3;
...
public class SyntaxSemanticAnalyser {
    ...
    public static boolean isWhileStatement(){
        ...
        index++;
        if (lexemes.get(index).getLexemeType() != LexemeType.UNTIL){
            printError("Ключевое слово until ожидалось в позиции " +lexemes.get(index).getPosition());
            return false;
        }
    ...
    private static void printEntryList(){
        System.out.println();
        System.out.println("Исходное выражение: " + LexAnalyser.inputString);
